...
export default function TextWithImage({ text, url, position, isDynamic, node }: TextWithImageSection) {
    return (
        <section
            className={`flex ${position === "Obrázek vlevo" ? "flex-wrap-reverse" : "flex-wrap"} md:flex-nowrap h-screen ${position === "Obrázek vlevo" ? "md:flex-row-reverse" : "md:flex-row"} p-10 lg:p-20 h-max`}>
            ...
            <div className={`w-full md:w-1/2 flex justify-between items-center py-4`}>
                <div className={`relative w-full h-[36rem] md:h-96 `}>
